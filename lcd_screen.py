...
class ST7920:
    ...
    def countdown(self, start_time, duration, pos):
        end_time = start_time + duration
        GPIO.output(self.CS_PIN, 1)
        self.send(self.LCD_BASIC)
        self.send(pos)
        print(f"Print: Coundtown for {duration} seconds")
        while end_time < time.time():
            rem_time = end_time - time.time()
            rem_time_str = f"{rem_time:.1f}"
            for element in range(0, 18):
                if element < len(rem_time_str):
                    self.data(ord(rem_time_str[element]))  # 0x41
                else:
                    self.data(ord(" "))
            time.sleep(0.2)
        GPIO.output(self.CS_PIN, 0)
